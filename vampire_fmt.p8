...
ct:bs(ct.fa)
ct.fa:fj(10)
bc,be,dw,ct.cq,iq,ju,ft,fu=4,0.15,2.5,6,6,0,0,0
ho,hr,jv,cx,jw,jx,jy=0,0.25,{},0,jt("001121562493d52e"),0,0.01
jz,ka,kb,kc,cp,di,dy,hi,ji,kd,ke,kf=0,-20,true,true,0,false,0,0,{{38,17}},0,0,0
ct:n()
end
function jt(r)
m={}
...
kj={fv,ga,ih,ik,hv:l({hx=24}),hv:l({hw=24}),hv:l({hx=-24}),id,ip,ir,gj,gj:l({gk=false}),gq,gf,il,iv,iz,jb,gz,jd,jd:l({jg=true}),hb,hd,jm,hh,ir:l({is=33})}
function kk(kl,km)
cls()
ki()
di,dy=kl==1,0
...
gg=kl
kl=jj[kl]
r,jq,jr,jo,jp,jl,jf,jh,eg,dd=kl.jn,kl.jq or jo,kl.jr or jp,kl.jo or jo,kl.jp or jp,kl.jl or 1,kl.jf or jf,kl.jh or jh,kl.gw or 0,{}
if gg==7 and kn then
jq,jr=540,186
elseif gg==5 and ko then
jr=34
end
if(kl.js)js=kl.js
ck,d,cursor,b,c,kp,kq=jt(sub(r,2,8)),kr(sub(r,9,10)),11,0,0,0,64
while cursor<#r or kp!=0 do
if kp<=0 then
ks=sub(r,cursor,cursor)
if ks=="/"then
kq=192-kq
elseif ks==":"or ks==";"then
...
ct.dn,ct.q,ct.kw,ct.f,ct.q,bw.dg,ct.r=0,false,0,0,false,false,1
end
ct:br()
bw:cy()
bw:cw()
...
function kg(kx)
for v=0,63 do
if kh("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!?",v+1)==kx then
return v
end
end
end
function kr(ky)
local kz,jg=kg(kh(ky,1)),kg(kh(ky,2))
return jg+kz*32
end
function kh(r,v)
return sub(r,v,v)
...
function _update60()
ef,dv=btn(5)and not la,btn(4)and not lb
la,lb=btn(5),btn(4)
...
function eu(b,c,lg)
return fget(mget(b,c),lg)
end
function bp(gr,lh,cr)
if gr+cr<lh then
return lh-cr
elseif gr-cr>lh then
return lh+cr
else
return gr
end
end
function ge(li,lj,lk,ll)
local lm,ln=abs(li-lk),abs(lj-ll)
return sqrt(lm*lm+ln*ln)
end
function _draw()
if jx!=0 and lo==jx and not lc then return end
lo=jx
cls()
if lc and not io then
lp()
lq("deaths: "..kd,72,7)
local he=""
if kf<10 then
...
lq("time: "..ke..":"..he..flr(kf),82,7)
if(cz)lq("hard difficulty",92,7)
lq(ho.."/3  ",116,7)
spr(58,68,114)
else
if cx<=0 then
...
spr(180,40+lm,72+ln)
spr(181,79-lm,72+ln)
lq("mush101.itch.io",118,5)
end
function lp()
local lw={0,8,0}
if not lc then
for v=1,3 do
rect(v-1,63+v,128-v,128-v,lw[v])
end
end
...
for ha in all(ji)do
ly,lz=ha[1],ha[2]
if lx and(hs<cp or hi%0.2<0.1)then
line(ly,lz,lx[1],lx[2])
end
hs+=1
circfill(ly,lz,1,8)
lx=ha
...
function lq(ma,c,mb)
print(ma,64-2*#ma,c,mb)
end
function lv()
for v=0x6000,0x7fff do
local mc=peek(v)
local md=mc%16
local me=lshr(mc-md,4)%16
me,md=jw[me+1],jw[md+1]
poke(v,shl(me,4)+md)
end
end
__gfx__
...

...

